...
if (!empty($_POST)) {
  ...
  header("location:status.php"); // your current page
}
...
if (!file_exists(dirname(__FILE__) . '/status.sql3')) {
  $_SESSION["status_file"] = 0;
} else {
  $_SESSION["status_file"] = 1;
  // $status = new adu_status($_SESSION["ADU"], "MT", sys_get_temp_dir() . '/status.sql3');
  $sys = new adu_status($_SESSION["ADU"], "MT", dirname(__FILE__) . '/status.sql3');
}
...
<body>
  ...
  <div class="w3-main" style="margin-left:50px">
    ...
    <div class="w3-row w3-padding-32">
      ...
      <?php
      foreach ($sys->channels as &$channel) {
